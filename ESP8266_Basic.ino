#include <SoftwareSerial.h>                         //including the SoftwareSerial library will allow you to use the pin no. 2,4 as Rx, Tx.
SoftwareSerial esp8266(2,4);                        //set the Rx ==> Pin 2; TX ==> Pin3. Connect Rx ==> Rx & Tx ==> Tx. Pin 3 has PWM thus kept for use.
...
int L1 =12;                                     //assign a variable named "L1" with an integer value 12, it refers to the pin which the red LED is connected on.
int L2 =11;                                    //assign a variable named "L2" with an integer value 11, it refers to the pin which the blue LED is connected on.


void setup()

{
  pinMode(L1,OUTPUT);                                 //set the pin number 12 as an output pin.
  pinMode(L2,OUTPUT);                               //set the pin number 11 as an output pin.
  
  digitalWrite(L1,LOW);                              //turn the L1 off at the beginning of the program.
  digitalWrite(L2,HIGH);                            //turn the L2 on at the beginning of the program.
  ...
  digitalWrite(L2,LOW);                             //after finishing the initialization successfully, turn off the blue LED (just an indicator).
}
